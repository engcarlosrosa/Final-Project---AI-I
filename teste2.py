...
# para o dataset
print(__doc__)
lista1 = [ f for f in listdir("../FaceRecognition") if isfile(join("../FaceRecognition",f))]   # pasta que armazena o dataset
lista_photos = []
lista_photos2 = []
lista_rgb = []
values = []    # dataset
...
img_file = '../FaceRecognition/webcam_rec/2015-05-29-090106.jpg'
print (img_file)


# foto da webcam
lista_webcam = [ f for f in listdir("../FaceRecognition/webcam_rec") if isfile(join("../FaceRecognition/webcam_rec",f))]   # pasta webcam
...
with open("arthur.csv", 'wb') as csvf:  # aqui comecerei a escrever os dados em arquivo csv
    ...
    for filename, values in numero.items():
    	...
    	elif filename.startswith("Lucas"):
    		csvwriter.writerow(["3"] + values)
...
#y.astype(int)
target_names = np.array(["Arthur Lee","Carlos Rosa", "Lucas C."])
n_classes = 3
